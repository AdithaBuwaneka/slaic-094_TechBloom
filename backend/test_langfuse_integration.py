#!/usr/bin/env python3
"""
Test script for Langfuse integration with the multi-agent travel system
"""
...
def test_workflow_integration():
    """Test the workflow integration with Langfuse"""
    ...
    try:
        ...
        # Check if Langfuse callbacks are integrated
        if hasattr(workflow, 'config') and 'callbacks' in workflow.config:
            print(f"   ✅ Langfuse callbacks integrated")
        else:
            print(f"   ⚠️  Langfuse callbacks not detected in workflow")
...
def test_environment_config():
    ...
    print("   Optional variables (Langfuse):")
...
def main():
    ...
    print(f"   Workflow Integration: {'✅' if workflow_ok else '❌'}")
