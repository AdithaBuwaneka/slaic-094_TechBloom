...
async def run_travel_agent(source: str, destination: str, mode: str, 
    ...
    try:
        ...
        if hasattr(final_state, 'current_step'):
            ...
            # If it's a dict, get values with defaults
            current_step = final_state.get('current_step', 'unknown')
            agents_completed = final_state.get('agents_completed', [])
            errors = final_state.get('errors', [])
            final_response = final_state.get('final_response', {})
        
        print(f"Workflow execution completed. Final step: {current_step}")
        ...
        if errors:
            for error in errors:
                if isinstance(error, dict):
                    print(f"   Error in {error.get('node', 'unknown')}: {error.get('error', 'unknown error')}")
                else:
                    print(f"   Error: {error}")
        
        if not final_response:
            print("No final response was generated by the workflow")
